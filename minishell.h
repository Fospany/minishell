...
int						check_for_quote_without_quote_type(const char letter);
void					get_real_quote_type(char *word, int *quote_type,
							int *i);
t_envs					*env_list_addition(t_token *tokens, t_envs *env_list);
...
char					*expand_env_assign(char *expandable, t_envs *env_list,
							int *index);
char					*get_full_expandable_word(t_token curr_token,
							t_envs *env_list, int len);
int						get_full_len_of_expandable(t_token curr_token,
							t_envs *env_list);
int						digit_counter(pid_t pid);
...
>>>>>>> 8f38889308c8472376f4055cb495304a9d5c97dd
