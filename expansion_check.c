...
char    *make_expansions(char *expandable, t_envs *env_list)
{
    char    *fully_expanded;

    fully_expanded = malloc(sizeof(char) * (get_full_len_of_expandable(expandable, env_list) + 1));
    create_the_whole_word(expandable, env_list, fully_expanded);
...
void    handle_expansions(t_envs *env_list, t_token *tokens)
{
    ...
    while (tokens[i].value)
    {
        if (dollar_in_word(tokens[i].value))
            tokens[i].value = make_expansions(tokens[i].value, env_list);
        i++;
    }
}
